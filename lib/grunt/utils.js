...
module.exports = {
  ...
  getVersion: function(){
    ...
    if (hash === 'fatal: Needed a single revision') hash = 'snapshot';
  ...
  startKarma: function(config, singleRun, done){
    ...
    var watch = hasFlag('--watch');
    var reporters = grunt.option('reporters');
    var noColor = grunt.option('no-colors');
    var debug = hasFlag('--debug');
    var port = grunt.option('port');
    if (watch) singleRun = false;
    var args = ['node_modules/karma/bin/karma', 'start', config];
    if (singleRun) args.push('--single-run=true');
    if (watch) args.push('--autowatch');
    if (reporters) args.push('--reporters=' + reporters);
    if (browsers) args.push('--browsers=' + browsers);
    if (noColor) args.push('--no-colors');
  ...
  process: function(src, DROP_VERSION, strict){
    var processed = src
      .replace(/"DROP_VERSION_FULL"/g, DROP_VERSION.full)
      .replace(/"DROP_VERSION_MAJOR"/, DROP_VERSION.major)
      .replace(/"DROP_VERSION_MINOR"/, DROP_VERSION.minor)
      .replace(/"DROP_VERSION_DOT"/, DROP_VERSION.dot)
      .replace(/"DROP_VERSION_CDN"/, DROP_VERSION.cdn)
      .replace(/"DROP_VERSION_CODENAME"/, DROP_VERSION.codename);
  ...
  build: function(config, fn){
    ...
    //process
    var processed = this.process(src, grunt.config('DROP_VERSION'), config.strict);
    if (styles) {
  ...
  //rewrite connect middleware (required for html5Mode, which is not being used)
  rewrite: function(){
    return function(req, res, next){
